<template lang="html">
  <button
    ref="btn"
    v-bind="$attrs"
    :class="[`vl-button-${isColor()?color:null}`,`vl-button-${type}`,{
      'isActive':isActive,
      'includeIcon':icon,
  ...
  >
    <span
      ...
      class="vl-button-backgroundx vl-button--background"
    />
    ...
    <VlIcon
      ...
      :icon-pack="iconPack"
      :icon="icon"
      class="vl-button--icon "
    />

    <span
      v-if="$slots.default"
      class="vl-button-text vl-button--text"
    >
      <slot />
    </span>
