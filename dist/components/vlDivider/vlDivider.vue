<template lang="html">
  <div class="vl-component vl-divider">
    <span
      :class="borderClass"
      :style="afterStyle"
      class="vl-divider-border after"
    />
    <span
    ...
    >
      ...
      <VlIcon
        v-else
        class="icon-divider notranslate vl-divider--icon"
        :icon-pack="iconPack"
        :icon="icon"
      />
    </span>
    <span
      :style="beforeStyle"
      :class="borderClass"
      class="vl-divider-border before"
    />
  </div>
</template>
  ...
  computed:{
    ...
    borderColor() {
      if (!_color.isColor(this.color)) {
        ...
        return _color.getColor('rgba(0, 0, 0,.1)')
      }
    },
    afterStyle() {
      const classes = {
    ...
    borderClass() {
      const classes = {}
      if (_color.isColor(this.color)) {
        classes[`vl-divider-border-${this.color}`] = true
      }
      return classes
    },
    textColor() {
    ...
    textClass() {
      const classes = {}
      if (_color.isColor(this.color)) {
        classes[`vl-divider-text-${this.color}`] = true
      }
      return classes
    }
  }
