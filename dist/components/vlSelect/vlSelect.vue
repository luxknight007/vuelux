  ...
  >
    ...
    <div class="input-select-con">
      ...
      <input
        ...
        class="input-select vl-select--input"
        type="text"
        @click.stop
      ...
      <VlIcon
        :icon-pack="iconPack"
        :icon="icon"
        class="icon-select vl-select--icon"
      />
      ...
      <Transition name="fadeselect">
        <div
          ...
          ref="vlSelectOptions"
          :style="cords"
          :class="[`vl-select-${color}`,{'scrollx':scrollx}]"
...
export default {
  ...
  computed:{
    ...
    listeners(){
      return {
        ...this.$listeners,
        blur: (event) => {
          if(this.autocomplete && event.relatedTarget?!event.relatedTarget.closest('.vl-select--options'):false ){
            this.closeOptions()
          }
  ...
  methods:{
    ...
    focus(){
      ...
      if (!this.autocomplete) {
        if(this.multiple?this.value.length == 0:!this.value || this.multiple){
          setTimeout( () => {
            this.$children[0].$el.querySelector('.vl-select--item').focus()
          }, 50);
        }
      }
      this.$nextTick(()=>{
    ...
    clickBlur(event){
      let closestx = event.target.closest('.vl-select--options')
      if(!closestx){
        this.closeOptions()
        if(this.autocomplete){
          this.filterItems('')
    ...
    changePosition(){
      ...
      let content = this.$refs.vlSelectOptions
