<template>
  <div
    ...
    :class="[`vl-pagination-${color}`]"
    class="con-vl-pagination"
  >
    <nav class="vl-pagination--nav">
      <button
        :class="{disabled:current <= 1 ? 'disabled' : null}"
        :disabled="current === 1"
        class="vl-pagination--buttons btn-prev-pagination vl-pagination--button-prev"
        @click="prevPage"
      >
        <VlIcon
          :icon-pack="iconPack"
      ...
      <ul class="vl-pagination--ul">
        <li
          v-for="(page, index) in pages"
          :key="index"
          :class="{'is-current': page == current}"
          class="item-pagination vl-pagination--li"
          @click="goTo(page)"
        >
          <span>
            {{ page }}
      ...
      <button
        ...
        class="vl-pagination--buttons btn-next-pagination vl-pagination--button-next"
        @click="nextPage"
      >
        <VlIcon
          :icon-pack="iconPack"
      ...
      <input
        ...
        v-model="go"
        :max="total"
        class="vl-pagination--input-goto"
        min="1"
        type="number"
        @change="goTo"
      >
